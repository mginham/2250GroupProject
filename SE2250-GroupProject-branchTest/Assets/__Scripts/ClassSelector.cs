...
public class ClassSelector : MonoBehaviour
{
    ...
    {
        if ((classnum == 0) && (Input.GetKeyDown(KeyCode.B)))//if the user selected the Crusador class and presses B then it makes the player grow and then shrink back to normal size
        {
            player1.transform.localScale+=new Vector3(1, 1, 1);
            print("Super Strength Activated!");
            Invoke("Shrink", 5f);
        }
        if ((classnum == 1) && (Input.GetKeyDown(KeyCode.B)))//if the user selected the Mage class and presses B then it makes the user invisible for a few seconds
        {
        ...
        if ((classnum == 2) && (Input.GetKeyDown(KeyCode.B)))//if the user selected the Archer class and presses B then the user swiftly side steps enemies
        {
